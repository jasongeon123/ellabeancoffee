...
export default async function ReturnsPage() {
  ...
  try {
    ...
    const ordersResult = await pool.query(
      ...
      WHERE o."userId" = $1 AND o.status IN ('completed', 'delivered')
      GROUP BY o.id
                      ...
                      <div className="text-right">
                        ...
                        {returnRequest.refundAmount && (
                          <p className="text-sm text-coffee-600 mt-2">
                            Refund: ${parseFloat(returnRequest.refundAmount).toFixed(2)}
                          </p>
                        )}
                      </div>
                    </div>
              ...
              <div className="grid gap-6">
                {orders.map((order: any) => {
                  ...
                  const hasActiveReturn = returns.some(
                    (r: any) => r.orderNumber === order.orderNumber && ["pending", "approved"].includes(r.status)
                  );
